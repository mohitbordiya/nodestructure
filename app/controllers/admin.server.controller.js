...
exports.adminProfile = (req,res)=>{
    ...
    Admin.find({_id:req.body.id}).exec((err, adminDetail)=>{
        if(err){
            res.send({status:0, message:err});               
        }else{
            res.send({status:1, message:'Admin detail', data:adminDetail});
...
Admin.findByIdAndUpdate(req.body.id, req.body, {new: true}).exec((err, data)=>{
    if(err){
        res.send({status:0, message:err});               
    }else{
        res.send({status:1, message:'Admin detail updated', data:data});
    }
});
// Admin.find({_id:req.body.id}).exec((err, adminDetail)=>{
//     if(err){
